...
(function () {
  ...
  const facts = [
    ...
    { icon: '🗣️', title: 'SSB с шагом 10 герц — редкость для карманного радио',
      text: 'Большинство карманных приёмников либо не умеют SSB вовсе, либо подстраивают только в узких пределах. PL-330 крутит частоту по всему КВ-диапазону с шагом 10 Гц — как у «больших» радиолюбительских трансиверов. Поэтому голос радиолюбителя на нём можно настроить до полностью естественного звучания.' },
    { icon: '🤫', title: 'Калибровка, которой «нет»',
      text: 'Производитель сначала отрицал, что в PL-330 можно откалибровать SSB, а потом признал: удержание USB или LSB при нулевых биениях сохраняет поправку. В инструкции об этом ни слова — узнали радиолюбители на форумах.' },
    { icon: '🎧', title: 'Стерео — только в наушниках',
      text: 'Через встроенный динамик FM всегда звучит в моно, а стерео включается, когда вставлены наушники. Динамик один — стерео ему всё равно ни к чему, а моно слышно чище.' },
    { icon: '🕰️', title: 'Приёмник знает, сколько он проработал',
      text: 'Если при выключенном радио подержать кнопку M три секунды, на экране появится, сколько часов приёмник проработал от аккумулятора, а ещё одно нажатие покажет общее время. Так можно понять, пора ли менять батарею.' },
    { icon: '🇩🇪', title: 'Grundig: 325 марок и 145 часов',
      text: 'Второй приёмник в доме, Grundig Concert-Boy N210, стоил в 1970 году 325 марок — треть месячной зарплаты рабочего. Зато шести батареек ему хватало на 145 часов — почти на месяц вечерних новостей.' },
  ...
  const quiz = [
    ...
    { q: 'Радиолюбитель говорит на 14.150 МГц. Какую кнопку нажать, чтобы голос стал нормальным?',
      a: ['USB', 'LSB', 'FM/ST'], c: 0,
      why: 'Выше 10 МГц — USB, ниже — LSB. Потом STEP до 10 Гц и медленно подстроить колесом.' },
    { q: 'Что означает буква U на кнопках старого Grundig?',
      a: ['Универсальный режим', 'УКВ — обычное FM-радио', 'Ультразвук'], c: 1,
      why: 'U — Ultrakurzwelle, «ультракороткие волны», то есть FM. L — длинные, M — средние, K — короткие.' },
    { q: 'В динамике треск и гул на коротких волнах. Что попробовать первым делом?',
      a: ['Купить новый приёмник', 'Отключить зарядку и отойти от техники', 'Нажать DELETE'], c: 1,
      why: 'Чаще всего шум приходит из квартиры: зарядка, светодиодные лампы, роутер. У окна и от аккумулятора эфир гораздо чище.' },
  ];

  views.quiz = {
    id: 'quiz', icon: '🎯', title: 'Небольшая викторина',
    render() {
      return `<p class="lead">Девять простых вопросов по мотивам фактов. Без таймера и без оценок — просто для интереса.</p>
  ...
  const subTiles = (current) => `
      <nav class="subtiles stg" aria-label="Карточки раздела Grundig">
        ${[
          ['grundig', '📻', 'О приёмнике'],
          ['g-use', '▶️', 'Как им пользоваться'],
          ['g-tune', '📡', 'Как ловить станции'],
          ['g-power', '🔌', 'Сеть или батарейки'],
          ['grundig-lamp', '💡', 'Не горит подсветка шкалы'],
          ['g-pointer', '🧵', 'Стрелка шкалы не двигается'],
          ['g-controls', '🔤', 'Словарь немецких надписей'],
          ['g-connect', '🎵', 'Слушать телефон через Grundig'],
          ['g-scale', '📻', 'Станции на шкале сегодня'],
          ['grundig-care', '🛠️', 'Уход за старой техникой'],
          ['g-value', '🏷️', 'Возраст и ценность'],
  ...
  views['grundig-care'] = {
    id: 'grundig-care', cat: 'museum', icon: '🛠️', title: 'Уход за старой техникой',
    render() {
      return `
  ...
  views['grundig-more'] = {
    id: 'grundig-more', cat: 'museum', icon: '📚', title: 'Узнать больше и документы',
    render() {
      return `
